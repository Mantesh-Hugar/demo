...
public class WebActionDriver implements ActionDriver {
	...
	// Pass parameter of browser value in Base class
	@Override
	...
	public void implicityWaitTime(int timeInSec) {
		...
		extentManager.logInfo("Implicity wait is " + timeInSec);
	}

	@Override
	public void getURL(String url) {
		extentManager.logging_Info("Entered URL :" + url);
		driver.get(url);
	...
	public String isElementDisplayed(By ele) {
		...
		if (getElement(ele).isDisplayed()) {
			extentManager.logging_Info("Given element is Displayed in Webpage");
			flag = "true";
		} else {
			extentManager.logging_Info("Given element is not Displayed in Webpage");
			flag = "false";
		}
		return flag;
...
//		highlighter(ele);
	...
	js.executeAsyncScript("arguments[0].setAttribute('style','background: yellow;border: 2px solid red;');", element);
	extentManager.logging_Info("Highlighting the clicked buttons");
}
